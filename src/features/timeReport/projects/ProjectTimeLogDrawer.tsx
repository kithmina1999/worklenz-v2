  ...
  return (
    <Drawer width={736} open={isTimeLogDrawerOpen} onClose={() => dispatch(toggleTimeLogDrawer())} title={<Typography.Title level={5}>{selectedLabel}</Typography.Title>}>
        ...
        {filteredData.map((logItem) => (
            <div>
                <Card className="time-log-card" title={<Typography.Text style={{ fontWeight: 500, fontSize: '16px', overflowWrap: 'break-word' }}>{formatDate(logItem.log_day)}</Typography.Text>} key={logItem.log_day}>
                    <Timeline>
                        {logItem.logs.map((log, index) => (
                            <Timeline.Item key={index}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    <Avatar style={{ backgroundColor: avatarNamesMap[log.user_name.charAt(0)], width: '26px', height: '26px' }}>
